                            ...
                            <div class="card-body">
                                <div class="table-responsive table-invoice">
                                    <table class="table table-striped" id="dataTable">
                                        ...
                                        @if(isset($helpCenterLists))
                                        <tbody>
                                            {{-- @foreach ($data as $index => $copyright) --}}
                                                <tr>
                                                    <td>1.</td>
                                                    <td>{{ $helpCenterLists->title }}</td>
                                                    <td>
                                                        @if ($helpCenterLists->image)
                                                            <img src="{{ asset($helpCenterLists->image) }}" alt=""
                                                                width="80px">
                                                        @endif
                                                    </td>
                                                    <td>{!! $helpCenterLists->description !!}</td>
                                                    <td>
                                                        {{-- <a href="{{ route('admin.customer-show', $customer->id) }}"
                                                            class="btn btn-primary btn-sm"><i class="fa fa-eye"
                                                                aria-hidden="true"></i></a> --}}
                                                        <a href="{{ route('admin.help-center-lists-edit', $helpCenterLists->id) }}"
